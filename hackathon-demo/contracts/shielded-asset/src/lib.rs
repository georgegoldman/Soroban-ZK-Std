...
use ethnum::u256;
use soroban_sdk::{contract, contractimpl, contracttype, Address, Bytes, Env, Symbol, U256};
use soroban_zk_core::G1Affine;
use soroban_zk_std::groth16::{Groth16Proof, Groth16VerifyingKey};
...
#[contractimpl]
impl ShieldedAsset {
    /// Initialise the contract with the address of the token it bridges.
    /// For the hackathon demo this is the native (XLM) token's Stellar Asset
    /// Contract, wrapped as a Soroban token. Must be called exactly once after
    /// deployment; `shield`/`unshield` panic until this has run.
    /// (Pre-existing design from #340: keep network-specific addresses out of
    /// the source so the contract is portable across networks.)
    pub fn initialize(env: Env, native_token: Address) {
        assert!(
            !env.storage()
                .instance()
                .has(&Symbol::new(&env, "NativeToken")),
            "already initialized"
        );
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "NativeToken"), &native_token);
    }
    ...
    pub fn shield(env: Env, user: Address, amount: i128, delta: EncryptedBalance) {
        ...
        soroban_sdk::token::Client::new(&env, &native).transfer(
            &user,
        ...
        env.storage()
            .persistent()
            .set(&user, &bal.combine(&env, &delta));
...
const VERIFYING_KEY_IC: &[G1Affine] = &[
    G1Affine {
        x: u256::from_words(
            0xa11cbd92460f325207159536d80c9f44,
            0x582e95462d19ac2c084caac89c25ca0,
        ),
        y: u256::from_words(
            0x14f2f0f329e1ee1dd5b5f67adb53683a,
            0x02b72312bc4175c4ce29578f93a3cb04,
        ),
    },
    G1Affine {
        x: u256::from_words(
            0xea2aee8ab2a7ccd129e35144e1f29140,
            0x1ef18d20cc2ab8523244707ddbc9474,
        ),
        y: u256::from_words(
            0xbced3ef5782c6a4278f685e6851503a,
            0xb10d38e29cd74b98ced8521f5f174a17,
        ),
    },
];
    ...
    Groth16VerifyingKey {
        alpha_g1: G1Affine {
            x: u256::from_str_radix(
                "6052c6ae90b77a962b3b355cf88cff3f084dfb2a423bd020222acd3e477a214",
                16,
            )
            .unwrap(),
            y: u256::from_str_radix(
                "20d1bbde469078cd777dab43de968f8d5330297c23334b32fe9b78841f64b18",
                16,
            )
            .unwrap(),
        },
        beta_g2: G2Affine {
            x: (
                u256::from_str_radix(
                    "1d639914164bbf9f91fe66713fc79afd3d2bf21206b2ac265b3fa94075852760",
                    16,
                )
                .unwrap(),
                u256::from_str_radix(
                    "1535f84fa5582f2f37628109bc1441ad4911d9d9b22294516260d7e9a9659b38",
                    16,
                )
                .unwrap(),
            ),
            y: (
                u256::from_str_radix(
                    "537bca9350ea24d4599a146d91e6964cbe99de3159b49aa35a496953bd33d28",
                    16,
                )
                .unwrap(),
                u256::from_str_radix(
                    "278fdcd0f4eda04f64df74dddf21946f69d2eabee79fe18f4189b8782cacad19",
                    16,
                )
                .unwrap(),
            ),
        ...
        delta_g2: G2Affine {
            x: (
                u256::from_str_radix(
                    "10c58eeca2bb26cfa71c9c334d68f8075cf12d73d58ea25b5d0aee17152b31d2",
                    16,
                )
                .unwrap(),
                u256::from_str_radix(
                    "1e497761c3ab876d78d244ea8dc62bd747bc62de185a628978144d909d4a0462",
                    16,
                )
                .unwrap(),
            ),
            y: (
                u256::from_str_radix(
                    "1fdd7a52499f577befaea5705386a0a5028fbb314d575eb707fff2f93697923e",
                    16,
                )
                .unwrap(),
                u256::from_str_radix(
                    "15b53a8bc08343ab2a9388050710f5905c3e08d3a35c5dfb53ee0e0984f55ef5",
                    16,
                )
                .unwrap(),
            ),

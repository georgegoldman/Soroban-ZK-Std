...
$\mathbb{F}_{q^{12}}$ is constructed as a tower of three quadratic and cubic extensions
over the BN254 base field $\mathbb{F}_q$:
...
using the coefficient indexing from Section 2. Positions $a_{0,1}$, $a_{1,1}$, and
$a_{1,2}$ are zero. Only three $\mathbb{F}_{q^2}$ values are non-zero:
...
For $T_0 = f_0 \cdot \ell_0$ with $\ell_0 = (c_0, 0, c_2)$:

Let $f_0 = (x_0, x_1, x_2)$. The product in $\mathbb{F}_{q^6} = \mathbb{F}_{q^2}[v]/(v^3 - (u+1))$ expands to:

$$T_0 = \bigl(x_0 c_0 + (x_1 c_2 + x_2 \cdot 0)(u+1)\bigr) + \bigl(x_1 c_0 + x_0 c_2 + x_2 \cdot 0 \cdot (u+1)\bigr) v + \bigl(x_2 c_0 + x_0 \cdot 0 + x_1 c_2\bigr) v^2$$

Since the middle coefficient of $\ell_0$ is zero, this simplifies to:
...
For $T_1 = f_1 \cdot \ell_1$ with $\ell_1 = (c_3, 0, 0)$:

Let $f_1 = (y_0, y_1, y_2)$. Since $\ell_1$ has only one non-zero coefficient:
...
Compared to 18 $\mathbb{F}_{q^2}$-muls for a generic $\mathbb{F}_{q^{12}}$ product, sparse
multiplication achieves a **~28% reduction** in multiplications. Across the full
Miller loop this compounds into a significant instruction-count saving.
...
Steps:
  ...
  // --- Recombine ---
  // f * ell = (T0 + T1 * v_coeff) + (T2 - T0 - T1) * w
  // where v_coeff applies the tower non-residue for the w^2 = v relation

  lo = T0 + fq6_mul_by_v(T1)   // T1 * v uses the Fq6 non-residue twist
  hi = T2 - T0 - T1

  return (lo, hi)               // as Fq12 = lo + hi*w
~~~
...
where $\ell$ encodes the line evaluation at a Miller step. Correctness is guaranteed
when:
